...
void* Mission(void *t)
{
	...
	int* ignored_count = my_data->ignored_count;
  ...
  // cout << "Finish Reading single CET" << tid << endl;
    ...
    //cout << "Read SMK File" << tid << endl;
		...
		for (size_t j = 0; j < target_nodes.size(); j++)
		{
			...
			avg_sim += AVGSIM(source_node, target_nodes[j], *all_user, *all_Node, d_table_list, vt_flag, Spatial_Distance, sim_list);
		}
		bool avg_sim_less_than_k_simer = false;
		pthread_mutex_lock(&mutex);
		if (int(Simer_vec->size()) == topK)
		{
			if(avg_sim <= Simer_vec->back().second)
			{
			avg_sim_less_than_k_simer = true;
			++*ignored_count;
			}
		}
		pthread_mutex_unlock(&mutex);

		if (avg_sim_less_than_k_simer)
			continue;
		...
		else if(d > Simer_vec->back().second)
		{
			Simer_vec->push_back(make_pair(source_node, d)); //when simer_vec size == top_k , added new simer score and sort it,and then pop the less one;
			sort(Simer_vec->begin(), Simer_vec->end(), topk_compare);
			Simer_vec->pop_back();
		}
		pthread_mutex_unlock(&mutex);
...
int main(int argc,char* argv[])
{
	...
	string detail_file = argv[9];
	int NUM_THREADS = str2int(argv[10]);
	int prune_IIN = str2int(argv[11]);
	string SMK_directory = argv[12];
	string CET_location = argv[13]; // new add
	int Distance = str2int(argv[14]); // new add
	int vt_flag = str2int(argv[15]); // new add
	double Spatial_Distance = str2double(argv[16]);
	string SIM_directory = argv[17];
...
//reading target nodes
	...
	for(size_t i=0;i<target_nodes.size();i++)
		cout << target_nodes[i] << ",";
	cout << endl << "topK = " << topK << " ,delta = " << delta << endl;
...
//	vector<double> SIMER_vec; // record SIMER value of V_s
	...
	int count_user = 0;
	int mission_part = 0;
	//let low social dist cal first.
	CmpByDist cmpbydist;
	vector<pair<int,Node*> > all_user_vec(all_user.begin(), all_user.end());
	sort(all_user_vec.begin(), all_user_vec.end(), cmpbydist);
	for (size_t i = 0;i<all_user_vec.size();i++)
	{
		count_user++;
		if (all_user_vec[i].first == user_node)
			continue;
		if (all_user_vec[i].second->dist == 2147483647)
			continue;
		if (all_user_vec[i].second->dist > Distance)
			continue;
		user_vec[mission_part].push_back(all_user_vec[i].second);
		mission_part++;
...
/*	for(iter = all_user.begin();iter!=all_user.end();iter++)
	...
	for(int i = 0;i < NUM_THREADS;i++)
	{
		...
		arg_set[i].ignored_count = &ignored_candidate_count;
...
//////////////////////////////////////////////////
	...
	fstream f;
	f.open(topK_file.c_str(), ios::out);
	for (size_t i = 0; i<size_t(topK) && i != SIMER_VEC.size(); i++)
	...
	cout << "Ignored Candidate Count:" << ignored_candidate_count << endl;
}
