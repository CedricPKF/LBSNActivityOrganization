...
double IMDR(int vs, int vt, MetaGraph* metaGraph, map<int, double>& omega, vector<D_TABLE_information*>& d_table_list, map<pair<int, int>, CET_FILE_information*>& cet_file_list, map<int, Node*>& all_Node, double delta, int vt_flag, double Spatial_distance)
{
	...
	if((double(distinct_entity.size())*double(CET_Leaf_node_count.size()))!=0)
		...
		imdr = 0.0;
	//if (isnan(imdr))//this method will £££º£¬£¼cause Floating point exception.
		//imdr = 0.0;
	return imdr;
}
...
{
	double simer = 0.0;
	simer = IMDR(vs, vt, metaGraph, omega, d_table_list, cet_file_list, all_Node, delta, vt_flag, Spatial_distance) / (all_user[vs]->dist);
	return simer;
}
